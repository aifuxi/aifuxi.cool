...
const BASE_URL = 'http://localhost:9003/public/api';
...
export function getArticles() {
  ...
  }).then((res) => res.json()) as unknown as TotalResponse<Article[]>;
}

export function getArticleByFriendlyUrl(friendlyUrl: string) {
  return fetch(`${BASE_URL}/articles/${friendlyUrl}`, {
    method: HttpMethod.GET,
  }).then((res) => res.json()) as unknown as GeneralResponse<Article>;
}
...
export function getTags() {
  ...
  return fetch(`${BASE_URL}/tags${obj2QueryString(req)}`, {
    method: HttpMethod.GET,
  }).then((res) => res.json()) as unknown as TotalResponse<Tag[]>;
}
